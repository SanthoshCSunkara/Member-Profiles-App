...
export const MemberProfiles: React.FC<IComponentProps> = (props) => {
  const {
    ...
    imageListId,
    itemsPerPage,
    accentColor,          // may be undefined
    pageTitle,
    pageSubtitle
  } = props;

  // Coalesce a definite string for anything that requires it
  const accent = accentColor ?? '#114461';
  ...
  return (
    <div className={styles.wrapper} style={{ ['--accent' as any]: accent }}>
        ...
        <div className={styles.grid}>
          {page.map((p) => (
            <PortraitCard
              ...
              role={p.role}
              photoUrl={p.photoUrl}
              onClick={() => setActive(p)}  // keep your details panel behavior
            />
          ))}
        </div>
      )}
      ...
      <DetailsPanel
        ...
        accentColor={accent}   // << now always a string
      />
    </div>
  );
};
